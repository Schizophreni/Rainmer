...
def prepare_outdoorrain():
    root_dir = "/home1/zhangsy/rh/data/derain/AllinOne/rain/train/"
    rains = glob.glob(os.path.join(root_dir, "input", "*.png"))
    gts = []
    for rain_img in rains:
        rain_name = rain_img.split("/")[-1]
        gts.append(os.path.join(root_dir, "target", rain_name))
    print("Outdoor Rain, total images: ", len(rains), len(gts))
    return rains, gts

def prepare_raindrop():
    root_dir = "/home1/zhangsy/rh/data/derain/AllinOne/raindrop/train/"
    rains = glob.glob(os.path.join(root_dir, "input", "*.png"))
    gts = []
    for rain_img in rains:
        rain_name = rain_img.split("/")[-1]
        gts.append(os.path.join(root_dir, "target", rain_name))
    print("RainDrop, total images: ", len(rains), len(gts))
    return rains, gts

def prepare_snow():
    root_dir = "/home1/zhangsy/rh/data/derain/AllinOne/snow/train/"
    snows = glob.glob(os.path.join(root_dir, "input", "*.jpg"))
    gts = []
    for snow_img in snows:
        snow_name = snow_img.split("/")[-1]
        gts.append(os.path.join(root_dir, "target", snow_name))
    print("Snow, total images: ", len(snows), len(gts))
    return snows, gts
...
class AnyRainDataset(data.Dataset):
    def __init__(self, opt):
        ...
        for dataset_name in opt.dataset_names.split(","):
            ...
            elif dataset_name.lower() == "outdoorrain":
                imgs, gts = prepare_outdoorrain()
            elif dataset_name.lower() == "raindrop":
                imgs, gts = prepare_raindrop()
            elif dataset_name.lower() == "snow":
                imgs, gts = prepare_snow()
            else:
    ...
    def __getitem__(self, index):
        ...
        for _ in range(self.opt.n_neg):
            blur_gts.append(self.detail_neg_transform(gt_img).unsqueeze(0))
