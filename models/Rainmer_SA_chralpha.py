...
class CrossAttention(nn.Module):
    def __init__(self, dim, base_dim, num_heads, bias, sparse_rates=None):
        ...
        self.qkv_dwconv = nn.Conv2d(dim * 3, dim * 3, kernel_size=3, stride=1, padding=1, groups=dim * 3, bias=bias)
        self.project_out = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)
        self.num_knowledge = self.project_out.weight.shape[1] // dim - 1
        self.attn_drop = nn.Dropout(0.)
        self.sparse_rates = sparse_rates
        if self.sparse_rates:
            self.attns = torch.nn.Parameter(torch.ones(len(self.sparse_rates))*0.2, requires_grad=True)
        ...
        self.chr_z = torch.nn.Parameter(torch.ones(1, base_dim, 1, 1, dtype=torch.float32), requires_grad=True)
    ...
    def forward(self, x, global_knowledge):
        ...
        if global_knowledge is not None:
            if len(global_knowledge) == 1:
                ...
                chr_alpha = F.normalize(self.chr_z) * global_knowledge[0].detach()
                chr_alpha = torch.sum(chr_alpha, dim=1, keepdims=True) # [b, 1, 1, 1]
                chr_alpha = (1 + chr_alpha) / 2.0
                prompt_detg = self.detg_promptAttn(out, global_knowledge[1])
                prompt_out = chr_alpha * prompt_chr + (1-chr_alpha) * prompt_detg
...
class CrossTransformerBlock(nn.Module):
    def __init__(self, dim, base_dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type, sparse_rates,
        ...
        self.norm1 = LayerNorm(dim, LayerNorm_type)
        self.attn = CrossAttention(dim, base_dim, num_heads, bias, sparse_rates)
        self.norm2 = LayerNorm(dim, LayerNorm_type)
        self.ffn = FeedForward(dim, ffn_expansion_factor, bias)

    def forward(self, x):
        x, global_knowledge = x
        x = x + self.attn(self.norm1(x), global_knowledge=global_knowledge)
        x = x + self.ffn(self.norm2(x))
