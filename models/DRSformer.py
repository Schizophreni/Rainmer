...
class Attention(nn.Module):
    def __init__(self, dim, num_heads, bias, sparse_rates=None):
        ...
        if self.sparse_rates:
            # self.attns = [torch.nn.Parameter(torch.tensor([0.2]), requires_grad=True) for _ in range(
            #     len(self.sparse_rates)
            # )]
    ...
    def forward(self, x):
        ...
        if self.sparse_rates:
            ...
            for rate_idx, rate in enumerate(self.sparse_rates):
                ...
                outs.append((attn @ v) * self.attns[rate_idx].to(x))
...
class ResBlock(nn.Module):
    def __init__(self, C_in, C_out, kernel_size, stride, padding):
        ...
        self.conv1 = nn.Conv2d(C_in, C_in, kernel_size=kernel_size, stride=stride, padding=padding,
                               bias=True)
        self.conv2 = nn.Conv2d(C_in, C_out, kernel_size=kernel_size, stride=stride, padding=padding, groups=C_in,
                               bias=True)
...
class DRSformer(nn.Module):
    def __init__(self, opt):
        ...
        num_blocks, heads = opt.num_blocks, opt.heads
    ...
    @torch.no_grad()
    def inference(self, x, im_q=None):
