...
class Experiments:
    def __init__(self, opt):
        ...
        # criterion
        self.criterion = SSIM()
        self.optimizer = torch.optim.Adam(params=self.model.parameters(), lr=opt.train.optim.lr, betas=(0.9, 0.999))
        self.scheduler = MultiStepLR(self.optimizer, milestones=opt.train.scheduler.milestones, gamma=0.1)
    ...
    def train(self):
        ...
        for epoch in range(self.init_epoch, self.epochs):
            ...
            self.model.train()
            tic = time.time()
            for iter, (input_train, target_train, _, _, _, _) in enumerate(self.train_loader):
                step += 1
                ...
                pixel_loss = -self.criterion(outs, target_train)
